...
class Gulash extends Shell
{
    ...
    boolean historyFull
    // used as a workaround for GROOVY-2177
    String evictedLine;
    // remembers the command which will get evicted if history is full

    Gulash(GroovyShell groovyShell, IO io, Closure registrar)
    {
        super(io)
        assert registrar
        parser = new Parser()
        interp = new Interpreter(groovyShell)
        registrar.call(this)
    }
    ...
    {
        return { shell ->
            def r = new XmlCommandRegistrar(shell, Groovysh.class.getClassLoader())
            r.register(Groovysh.class.getResource('commands.xml'))
        }
    }

    Gulash(GroovyShell groovyShell,
           final IO io)
    {
        this(groovyShell, io, createDefaultRegistrar())
    }
    ...
    private String buildPrompt()
    {
        ...
        def formattedPrompt = "@|bold gulash:|@${lineNum}@|bold >|@ "

        def GROOVYSHELL_PROPERTY = System.getProperty("gulash.prompt")
    ...
    File getUserStateDirectory()
    {
        ...
        def dir = new File(userHome, '.gulash')
    ...
    int run(final String commandLine)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                runner.historyFile = new File(userStateDirectory, 'gulash.history')
